...
import {DieselAmm, ReadonlyDieselAmm} from "disel-dex-ts";
...
const contractId = process.env.DIESEL_CONTRACT_ID;

const scalingFactor = new BN(10000);

export class DieselDex implements DexInterface {
    private readonly wallet: WalletUnlocked;
    private readonly amm: DieselAmm;
    private readonly readonlyAmm: ReadonlyDieselAmm;

    constructor(wallet: WalletUnlocked) {
        this.wallet = wallet;
        this.amm = new DieselAmm(wallet, contractId);
        this.readonlyAmm = new ReadonlyDieselAmm(wallet.provider, contractId);
    }
    ...
    async getTokenInfo(assetId: AssetId): Promise<TokenInfo> {
        ...
        const assetInfo = await retry(
            async () => await this.readonlyAmm.lpAssetInfo(lpAssetId)
        );
    ...
    async getSwapAmount(assetIn: AssetId, assetOut: AssetId, amount: BN): Promise<BN> {
        return this.handlePoolNotFound(
            async (pools) => this.readonlyAmm.previewSwapExactInput(assetIn, amount, pools),
            assetIn,
            assetOut
        ).then(([_, amountOut]) => {
        ...
        const txRequest = await this.handlePoolNotFound(
            async (pools) => {
                ...
                while (true) {
                    ...
                    try {
                        return retry(() => {
                            return this.amm.swapExactInput(
                                amount,
                                assetIn,
                                amountCalculated,
                                pools,
    ...
    async getRate(assetIn: AssetId, assetOut: AssetId): Promise<number> {
        return this.handlePoolNotFound(
            async (pools) => this.readonlyAmm.getCurrentRate(assetIn, pools),
            assetIn,
            assetOut
